...
uk_ev <- read_ods(here("raw_data/ev_by_la.ods"), skip = 5)
...
```{r}
    pal <- colorBin("Greens", domain = uk_ev_map$x2021_q1, bins = 10)
...
leaflet(uk_ev_map) %>% 
  ...
  addPolygons(data = uk_ev_map$geometry,
             # fillColor = ~pal(x2021_q1),
              weight = 0.5,
              opacity = 0.9, 
              color = "green",
              fillOpacity = 0,
