...
no2_diff_final <-  merge(no2_diff_row, latlon, by.x = "rowid", by.y = "rowid") %>%
  select(lat, lon, no2_diff_2014_2019) 
```


```{r}
    pal <- colorBin("Greens", domain = no2_difference$no2_diff_2014_2019, bins = c(0, 500, 1000, 2500, 5000, 10000, 15000))
    
    no2_diff_labels <- sprintf(
      "<strong>%s</strong><br/>%g NO2 Diff",
      no2_difference$uk_grid_code, no2_difference$no2_diff_2014_2019) %>% 
      lapply(htmltools::HTML)
```
...
no2_difference %>% 
  ...
  setView(lng = -4.2026, lat = 55.8, zoom = 4.7, options = list()) %>%
  addProviderTiles(providers$CartoDB.Positron) %>% 
  addPolygons(fillColor = ~pal(no2_diff_2014_2019),
    weight = 0.1,
    opacity = 0.9, 
    color = "black",
    fillOpacity = 0.8,
    highlightOptions = highlightOptions(color = "green", weight = 2,
                                        bringToFront = TRUE),
    label = no2_diff_labels,
    labelOptions = labelOptions(
      style = list("font-weight" = "normal", padding = "3px 8px"),
      textsize = "15px",
      direction = "auto")) %>% 
  addLegend(pal = pal, values = ~no2_diff_2014_2019, opacity = 0.7, title = NULL,
            position = "bottomright")
```